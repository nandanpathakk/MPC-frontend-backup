...
export const Demo = () => {
    const [selectedOptions, setSelectedOptions] = useState<string[]>(['email']); // State to store selected options with email pre-selected
    const [selectedColor, setSelectedColor] = useState<string>('#ffffff'); // State to store selected color

    // Function to handle selection of options
    const handleOptionSelection = (option: string) => {
        if (selectedOptions.includes(option)) {
            setSelectedOptions(selectedOptions.filter((item) => item !== option)); // Deselect option if already selected
        } else {
            setSelectedOptions([...selectedOptions, option]); // Select option if not selected
        }
    };
                    ...
                    <div className="flex h-full">
                        ...
                        <div className="w-1/2 p-8 flex justify-center items-center">
                            ...
                            >
                                ...
                                {/* Conditional Rendering for Social Media Options */}
